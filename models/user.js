...
User.findByEmail = (email) => {
  const sql = `
    ...
    GROUP BY U.id
  `;
  return db.oneOrNone(sql, [email]); // <== IMPORTANTE: pasar como arreglo
};
...
User.findById = (id) => {
  const sql = `
    ...
    WHERE id = $1
  `;
  return db.oneOrNone(sql, [id]); // <== IMPORTANTE: pasar como arreglo
};

User.create = async (user) => {
  const hash = await bcrypt.hash(String(user.password), 10); // evita "undefined, number"
...
User.updateSessionToken = (id_user, session_token) => {
  const sql = `
    ...
    WHERE id = $1
  `;
  return db.none(sql, [id_user, session_token]); // <== arreglo
};
